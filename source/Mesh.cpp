...
namespace dae
{
	Mesh::Mesh(ID3D11Device* pDevice, const std::string& path, BaseMaterial* pMaterial)
	...
	void Mesh::Render(ID3D11DeviceContext* pDeviceContext, Camera* pCamera)
	{
		...
		Matrix worldViewProjectionMatrix{ m_WorldMatrix * pCamera->GetViewMatrix() * pCamera->GetProjectionMatrix() };
		m_pMaterial->SetMatrix(worldViewProjectionMatrix, m_WorldMatrix, pCamera->GetInvViewMatrix());
